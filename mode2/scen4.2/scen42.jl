...
function CarSim(params::SimParams, models::Dict{Int, DriverModel})
    ...
    cars = [Vehicle(VehicleState(VecSE2(-35.0,0,0), roadway.segments[1].lanes[1],roadway, 11.17),
             VehicleDef(), 1), Vehicle(VehicleState(VecSE2(0.5,-2.0,π/2), env.crosswalk, cw_roadway, 1.4),
             PEDESTRIAN_DEF, 2),Vehicle(VehicleState(VecSE2(-0.5,-2.0,π/2), env.crosswalk, cw_roadway, 1.4),
             PEDESTRIAN_DEF, 3),Vehicle(VehicleState(VecSE2(0.5,-4.0,π/2), env.crosswalk, cw_roadway, 1.4),
             PEDESTRIAN_DEF, 4),Vehicle(VehicleState(VecSE2(-0.5,-4.0,π/2), env.crosswalk, cw_roadway, 1.4),
             PEDESTRIAN_DEF, 5)]
    car1 = cars[1]
    car2 = cars[2]
    car3 = cars[3]
    ...
    env.observed = [car2,car3,car4,car5]
...
function initialize(sim::CarSim)
    ...
    sim.roadway.observed = [sim.scene[2], sim.scene[3], sim.scene[4], sim.scene[5]]
    empty!(sim.states)
    if sim.p.logging
...
function run_sim(maxtime::Int64,s::Array{Float64,1},d::Int64,n::Int64,seed::Int64)
    ...
    models[3] = CrosswalkDriver(LaneSpecificAccelLatLon(0.0,0.0), s)
    models[4] = CrosswalkDriver(LaneSpecificAccelLatLon(0.0,0.0), s)
    models[5] = CrosswalkDriver(LaneSpecificAccelLatLon(0.0,0.0), s)
...
carcolors[4] = colorant"blue"
carcolors[5] = colorant"blue"
...
writedlm("scen11_rewards.txt", r_history)
    # reward = reward[1]
...
write(@sprintf("stress_test2_R_%f.gif",reward[1]), frames)
	# end
